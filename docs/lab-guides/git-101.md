...
1. **Clone your personal repo:**
    ...
    ```bash
    git clone https://gitlab.puppetlab.uoregon.edu/participantXX/myrepo.git
    ```

    You will be prompted for your participantXX credentials, enter them to clone the repository.
...
1. **Clone your group repo** (groups are 1-5, 6-10, etc.):
    ...
    ```bash
    git clone https://gitlab.puppetlab.uoregon.edu/groupN/ourrepo.git
    cd ourrepo
    ```

2. **Edit the file named after your participant number** so it won't conflict with your groupmates:

    ```bash
    echo "Hello from participantXX" > participantXX.md
    git add participantXX.md
    git commit -m "Update participantXX file"
    git push
    ```
...
1. **Everyone in the group edits the same file.** Open `README.md` and replace "raccoon" with your favorite animal.
2. **One person pushes first:**

    ```bash
    git add README.md
    git commit -m "Update my favorite animal"
    git push
    ```
...
| Requirement | Details |
|-------------|---------|
| Git remote | A Git server accessible to all participants (gitlab.puppetlab.uoregon.edu TBD) |
