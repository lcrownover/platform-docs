...
When a Puppet agent runs, here's what happens:

1. The server has a `pp_role` fact set (e.g., `puppet_dba::role::loire`)
2. The site manifest (`site.pp`) looks at the value of `pp_role` and includes that role
...
```puppet
class puppet_yourteam::role::myserver {
  ...
  include puppet_yourteam::profile::postgresql

  # 5. Server profile (optional, same name as role)
  include puppet_yourteam::profile::myserver
}
```

### The Includes, In Order

1. **System tags**: Required for integration with log aggregation and other services. This is some cruft, and needs to be moved elsewhere, but for now just copy the convention when you create new roles.
...
Naming the server profile the same as the role makes it easy to find. When you're looking at `puppet_yourteam::role::myserver`, you know the server-specific code is in `puppet_yourteam::profile::myserver`.

```puppet
# puppet_yourteam/manifests/profile/myserver.pp
class puppet_yourteam::profile::myserver {
  # Everything here is specific to this one server/cluster
  include puppet_yourteam::profile::sshkeys::specialuser
...
```puppet
# puppet_yourteam/manifests/role/myserver.pp

# MyServer - Production web application server
#
# This server runs the main customer-facing web application.
# It handles approximately 10k requests/hour during peak times.
#
# Contacts:
...
|------|---------|---------|
| `pp_role` | Which role class to include | `puppet_dba::role::daisy` |
...
└── myserver.pp                # puppet_yourteam::profile::myserver (server profile)
```
...
If you need to change something in the control repo (like `profile::base` or `common.yaml`), submit a PR to `puppet-control-repo`. These changes affect everyone, so they'll get extra review.

### Verifying Your Changes

After pushing changes to production, either run Puppet manually on an affected node or wait for the next scheduled run and check the results. You don't want to leave a node in a state where the catalog fails to compile.
...
**The failure is visible.** You'll see the error in the Puppet run output, and it shows up in Puppet Enterprise reporting. The node is marked as failing, which is your signal to investigate.
...
Most servers run in the `production` environment, which tracks the `master` (or `main`) branch. When your PR is merged to master, the Puppet server picks up the changes within a few minutes. Servers will apply the new configuration on their next Puppet run.
...
3. **Assign a test node to the new environment.** On the test node as root, run:
   ```bash
   set_temp_environment feature_new_nginx_config
   ```

4. **Test your changes.** Run Puppet on the test node and verify everything works.

5. **Merge to master.** Once you're confident, merge your feature branch into `master`.

6. **Switch nodes back to production.** On the test node:
   ```bash
...
| Set Hiera data for your servers | `data/` directory in your team module |
